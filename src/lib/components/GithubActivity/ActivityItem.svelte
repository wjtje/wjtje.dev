...
<div>
	...
	{#if event.type == 'CommitCommentEvent'}
		<h3>
			{@html $t(`GitHubActivity.${event.type}`, {
	...
	{:else if event.type == 'CreateEvent' || event.type == 'DeleteEvent'}
		<h3>
			{@html $t(`GitHubActivity.${event.type}.${event.payload?.ref_type ?? 'unknown'}`, {
				...event.payload,
				repo: event.repo.name,
				id: event.id
			})}
	...
	{:else if event.type == 'GollumEvent'}
		...
		<p>
			{($t(`GitHubActivity.${event.type}Subtitle`), { pages: event.payload?.pages?.length ?? 0 })}
		</p>
	{:else if event.type == 'IssueCommentEvent'}
		<h3>
			{@html $t(`GitHubActivity.${event.type}.${event.payload?.action ?? 'unknown'}`, {
	...
	{:else if event.type == 'IssuesEvent'}
		{#if event.payload?.action == 'opened' || event.payload?.action == 'edited' || event.payload?.action == 'closed'}
			...
			{#if event.payload?.action == 'opened' && event.payload?.issue?.body != null}
				<p>{event.payload?.issue?.body}</p>
			{/if}
		{/if}
	{:else if event.type == 'PublicEvent'}
		<h3>
			{$t(`GitHubActivity.${event.type}`, {
				repo: event.repo.name
			})}
		</h3>
	{:else if event.type == 'PullRequestEvent'}
		{#if event.payload?.action == 'opened' || event.payload?.action == 'edited' || event.payload?.action == 'closed' || event.payload?.action == 'reopened'}
			...
			<p>
				{@html $t(`GitHubActivity.${event.type}.subtitle`, {
	...
	{:else}
		<h3>{$t('GitHubActivity.UnknownEvent', { ...event })}</h3>
		<p>{$t('GitHubActivity.UnknownEventSubtitle', { ...event })}</p>
	{/if}
</div>
